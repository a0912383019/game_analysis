  ...
  [
    {
      ...
      align: 'center',
      customCell: (record, _) => {
        if (record.__index_in_group === 0) {
          return {
            rowSpan: record.__group_length
          }
        }
        return {
          rowSpan: 0
...
const transformBetReportByLobby = (
  ...
  console.log(record.innerPagination.current)
  ...
  if (record.innerPagination.current === 2) {
    data = [
      {
        member_id: '99281113',
        data: [
  ...
  record.innerData = data.flatMap((group) => {
    return group.data.map((item, index) => ({
      ...item,
      member_id: group.member_id,
      __index_in_group: index,
      __group_length: group.data.length
    }))
  })
}
