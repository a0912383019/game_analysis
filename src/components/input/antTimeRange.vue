...
const props = withDefaults(defineProps<AppTimeRangeProps>(), {
  defaultDates: () => [dayjs().startOf('day'), dayjs().endOf('day')], // 預設當天
...
const handleClick = (item: string) => {
  if (item === 'start') openStart.value = true
  if (item === 'end') openEnd.value = true
...
const okStart = () => {
  openStart.value = false
  if (!bindingEndValue.value) {
    openEnd.value = true
  }
}
const okEnd = () => {
  openEnd.value = false
  if (!bindingStartValue.value) {
